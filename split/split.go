...
type HandleStyle struct {
	Color      color.NRGBA
	Width      unit.Dp
	HoverColor color.NRGBA
}

// Layout displays w1 and w2
//
...
func (rs *Split) Layout(gtx layout.Context, w1, w2 layout.Widget) layout.Dimensions {
	...
	oldPos := rs.float.Pos
	m := op.Record(gtx.Ops)
	dims := rs.float.Layout(gtx, rs.Axis, rs.HandleStyle)
	c := m.Stop()
...
func handleLayout(gtx layout.Context, axis layout.Axis, style HandleStyle, hovered bool) layout.Dimensions {
	var maxDim image.Point
	if axis == layout.Vertical {
		maxDim = image.Pt(gtx.Constraints.Max.X, gtx.Dp(style.Width))
	} else {
		maxDim = image.Pt(gtx.Dp(style.Width), gtx.Constraints.Max.Y)
	}
	...
	paint.FillShape(gtx.Ops, style.Color, clip.Rect(line).Op())
