...
func (r *renderItem) layoutContent(gtx layout.Context, th *theme.Theme) layout.Dimensions {
	...
	contentColor := colors.MulAlpha(th.Base.Text, 200)
	if r.selected {
		contentColor = th.Base.Text
	}
	return layout.Inset{
		Left:  unit.Dp(2),
		Right: unit.Dp(2),
...
func (r *renderItem) layoutBackground(gtx layout.Context, th *theme.Theme) layout.Dimensions {
	...
	if r.hovering {
		fill = colors.MulAlpha(th.Base.Text, r.AlphaPalette.Hover)
	} else if r.selected {
		fill = colors.MulAlpha(th.Base.SurfaceHighlight, r.AlphaPalette.Selected)
	}
	rr := gtx.Dp(unit.Dp(5))
	defer clip.RRect{
